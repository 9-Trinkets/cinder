...
pub fn minimal_test_pack() -> ContentPack {
    ...
    let pack = load_pack_from_dir(&base).expect("load synthetic pack");
    fs::remove_dir_all(&base).expect("remove synthetic pack");
    pack
}

fn test_pack_dir() -> std::path::PathBuf {
    ...
    std::env::current_dir()
        ...
        .join("target")
        .join("reducer-test-packs")
        .join(format!("pack-{unique}"))
}
